...
begin
    ...
    if !isdir(SIG_DIR)
        md"Папка $SIG_DIR пока не создана. Делаем расчеты в консоли..."
    else
        files = filter(f -> endswith(f, ".jld2"), readdir(SIG_DIR))
        
        as_list = Float64[]
        fs_list = Float64[]
        for f in files
            # ИСПРАВЛЕННАЯ РЕГУЛЯРКА: добавлено \.jld2$ в конец!
            m = match(r"data_A_([0-9.]+)_F_([0-9.]+)\.jld2$", f)
            if m !== nothing
                push!(as_list, parse(Float64, m.captures[1]))
                push!(fs_list, parse(Float64, m.captures[2]))
            end
        end
        
        unique_A = sort(unique(as_list))
        unique_F = sort(unique(fs_list))
        
        md"Найдено дампов: $(length(files)). Доступные амплитуды (А): $unique_A"
    end
end
...
begin
    target_file = joinpath(SIG_DIR, "data_A_$(sel_A)_F_$(sel_F).jld2")
...
begin
    if ismissing(data)
        ...
        p_in = plot(t_us, data["signal_in"], 
            title="Входной импульс PZT (Давление)", ylabel="МПа", 
            color=:red, legend=false, lw=2, grid=true)
            
        p_out = plot(t_us, data["signal_out"], 
            title="Выходной сигнал на Микрофоне", xlabel="Время (мкс)", ylabel="МПа", 
            color=:blue, legend=false, lw=2, grid=true)
            
        plot(p_in, p_out, layout=(2,1), size=(800, 450))
    end
end
