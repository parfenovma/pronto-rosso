...
# ╔═╡ 84288d06-d3a9-11f0-bd81-391795ea99c2
using Gridap, GridapMakie, WGLMakie, PlutoUI; WGLMakie.activate!()

# ╔═╡ 657240be-b7de-4a8e-9fe5-8ef4d27482c3
begin
    # --- 1. Геометрия ---
    # Квадрат 2x2, разрешение 40x40 элементов
    domain = (0, 2, 0, 2)
    partition = (40, 40)
    model = CartesianDiscreteModel(domain, partition)
    
    # --- 2. Пространства (FE Spaces) ---
    # Граничные условия: 0 на границах (Dirichlet)
    reffe = ReferenceFE(lagrangian, Float64, 1)
    V = FESpace(model, reffe, conformity=:H1, dirichlet_tags="boundary")
    U = TransientTrialFESpace(V, t -> 0.0)
    
    # Сетка и триангуляция для отрисовки
    trian = Triangulation(model)
    degree = 2
    dΩ = Measure(trian, degree)
    
    # --- 3. Слабая форма (Weak form) ---
    # Уравнение: utt - Δu = 0
    # В Gridap формате: m(utt, v) + a(u, v) = 0
    m(t, utt, v) = ∫( utt * v )dΩ
    a(t, u, v)   = ∫( ∇(u) ⋅ ∇(v) )dΩ
    
    op = TransientFEOperator(m, a, U, V)
end

# ╔═╡ 36817a3d-5790-45fa-8d94-b8712710bfe1
begin
    # --- Начальные условия ---
    # Гауссов импульс в центре (1, 1)
    u0_func(x) = 1.5 * exp(-30 * ((x[1]-1)^2 + (x[2]-1)^2))
    v0_func(x) = 0.0 # Начальная скорость 0
    a0(x) = 0.0 # <--- Добавляем явное начальное ускорение (0)
    
    # Интерполяция начальных условий
    uh0 = interpolate_everywhere(u0_func, U(0.0))
    vh0 = interpolate_everywhere(v0_func, U(0.0))
    
    # --- Настройка решателя времени (Time Solver) ---
    t0 = 0.0
    tF = 3.0       # Конечное время
    dt = 0.04      # Шаг по времени
    
    # Схема Ньюмарка (gamma=0.5, beta=0.25 - безусловно устойчивая)
    ls = LUSolver()
    ode_solver = Newmark(ls, dt, 0.5, 0.25)
    
    # --- ГЛАВНОЕ ИСПРАВЛЕНИЕ ---
    # Передаем кортеж из ТРЕХ элементов: (смещение, скорость, ускорение)
    sol_ode = Gridap.solve(ode_solver, op, (uh0, vh0, ah0), t0, tF)
    
    # Собираем решения в массив, чтобы слайдер не тормозил вычисления
    # solutions - это вектор пар (время, решение)
    solutions = collect(sol_ode)
    
    "Решение вычислено: $(length(solutions)) шагов."
end

# ╔═╡ cf17e61e-a4fb-40b6-92b6-196fe64b3304


# ╔═╡ d761e9e3-a76e-406c-8f6b-23a7b733ca93
begin
	
end

# ╔═╡ 6eeb3d8a-ce4a-4e94-b7bb-23a7c970de74
# Просто вызовите plot или wireframe. 
# Объект Figure автоматически отобразится в ячейке.
plot(trian)


# ╔═╡ e8d82fb8-4eee-4a46-8909-7884d53c3800
...
manifest_format = "2.0"
project_hash = "4ee71e331e3579123cd3caf08483fa2a8bd57750"
...
git-tree-sha1 = "f2f1a514108c08f9e278e96972c3d50413bf5095"
...
git-tree-sha1 = "c104c72e37b5dfbeb88e203575c666061415b621"
...
[[deps.GridapMakie]]
deps = ["FileIO", "FillArrays", "GLMakie", "GeometryBasics", "Gridap", "Makie"]
git-tree-sha1 = "068c5f4675f70c0b64595db80e24ac4824f178f3"
...
deps = ["LinearAlgebra", "SparseArrays", "SuiteSparse"]
git-tree-sha1 = "cc05d16e340aecfa0e4cf4616194abc894cd0bca"
