...
# ╔═╡ f13f6d62-cf51-11f0-8d9b-5f7bc02a4759
using Gridap, Gridap.ReferenceFEs, LinearAlgebra, GLMakie

# ╔═╡ de227049-ac74-4edd-8621-8361b1a0a365
begin
	const E = 3.0e10 # Pa
...
# ╔═╡ b51d6086-e885-451d-a90f-df0930e158e4
begin
	const σ_u = 4.0e5 # Pa
...
# ╔═╡ 7c0d50ee-9365-47dd-b576-02509b8fdbe5
begin
...
# ╔═╡ a0ac07ea-c8de-4a82-878e-9f07fa3f9864
begin
...
# ╔═╡ 19dad869-be2a-4a2b-9a08-2edad38bbf43
const b_max = VectorValue(0.0,0.0,-(9.81*2.5e3))

# ╔═╡ 052ceaaf-3db5-4264-9de9-97c2ddba04bb
...
# ╔═╡ 2a6ddc99-fdea-4b54-872c-26bd65bf23cf
...
# ╔═╡ 417ec580-68a4-4bf2-a8d7-4f3c5cf736a0
main(n=8,nsteps=30)

# ╔═╡ ed3012b7-c53e-4c77-80f1-1e5799f81742
function visualize_damage()
    fig = Figure(resolution=(1000, 800))
    
    # Создаем слайдер для выбора шага
    slider = Slider(fig[2, 1], range=1:30, startvalue=1)
    step = slider.value
    
    # Загружаем данные для текущего шага
    damage = @lift begin
        step_val = $(step)
        filename = "output_path/results_$(lpad(step_val, 3, '0')).vtk"
        # Загрузите данные повреждения из файла
        # return loaded_damage_data
        zeros(10, 10)  # временная заглушка
    end
    
    # Создаем heatmap
    ax = Axis(fig[1, 1], title=@lift("Damage at step $($step)"))
    hm = heatmap!(ax, damage, colormap=:viridis, colorrange=(0, 0.5))
    Colorbar(fig[1, 2], hm)
    
    # Анимация
    record(fig, "damage_animation.mp4", 1:30) do i
        step[] = i
    end
...
# ╔═╡ 38e8410c-6dc9-47aa-b658-dab7ca826937
...
manifest_format = "2.0"
project_hash = "3427375db88ce0d73c881b7db438143ae6f82ce7"
